...
class dingding:
    ...
    def __init__(self, directory):
        ...
        self.adbquickbutton_dingding = '"%s\\adb" shell input tap %s' % (
        directory, config.get("position", "quickbutton"))
        self.adbquickopenbutton_dingding = '"%s\\adb" shell input tap %s' % (
        directory, config.get("position", "openquick"))
        self.adbnormalbutton_dingding = '"%s\\adb" shell input tap %s' % (
        directory, config.get("position", "normalbutton"))
        # 点击上班打卡
        self.adbclick_playgocard = '"%s\\adb" shell input tap %s' % (
        directory, config.get("position", "playgo_position"))
        ...
        # 传送到计算机
        self.adbpull = '"%s\\adb" pull sdcard/screen.png %s%s/screen-%s.png' % (
        directory, screen_dir, today, self.filetime)
        # 删除设备截屏
...
def start_loop(hourtype, minute):
    ...
    if hourtype == after_work_hour_type and now_hour == back_hour and now_minute == minute and is_weekend(today):
        ...
        elif go_hour < now_hour <= back_hour:
            # 9:00-17:00
        ...
        tip_msg = "now: {}:{} -- next_minute: {} -- only_minute: {} -- {} {}".format(now_hour, now_minute, minute,
                                                                                     only_minute,
                                                                                     is_weekend(today, False, False),
                                                                                     next_dktime_str)

        logger.info(tip_msg)
        # dingding(directory).wechatmsg2(tip_msg)
        if only_minute in [5, 10, 15, 30, 60, 500, 900, 1500]:
